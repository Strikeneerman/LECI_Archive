...
public class P53 {
	...
	public static void main(String[] args) {
		...
		if (!dis.addAluno(bls3))
			System.out.println(bls3.nMec() + ", " + bls3.getnome()+ " já está inscrito nesta disciplina!");
		
		if (dis.delAluno(bls2.nMec()))
			System.out.println(bls2.getnome() + " Removido!");
		...
		ReadWriteCheck[] list = new ReadWriteCheck[3];
		list[0] = new CSV("CSV.txt");
		list[1] = new Nokia("Nokia.txt");
		list[2] = new VCard("VCard.txt");
		
		for(ReadWriteCheck c: list){
			c.readCont(dis);
		}
